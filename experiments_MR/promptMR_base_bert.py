# PromptMR-base Manual Template and Manual Label Words Mapping Strategy

import argparse
from utils_MR import get_train_test_examples, calculate_metof1_literal_acc, set_logger, log_few_shot_examples, calculate_metof1_literal_acc_precisionandrecall
import os
...
def experiment_prompt_base(args):
    if not args.log_file:
        logger = set_logger(f'../results_MR/prompt_base/_{args.model_type}_{args.dataset}_{args.shot_count}_shot_@{datetime.now().strftime("%Y_%m_%d-%H_%M_%S")}.log')
    else:
    ...
    plm, tokenizer, model_config, WrapperClass = load_plm(args.model, args.model_type)  # load_plm("t5", "t5-base")
    if args.dataset != 'ChineseMR':
        template_text = '{"placeholder":"text_a"} So, {"placeholder":"text_b"} is a {"mask"} entity.'
        label_words = [["metonymic"], ["literal"]]
        myverbalizer = ManualVerbalizer(tokenizer, num_classes=2, label_words=label_words)
    else:
        template_text = '{"placeholder":"text_a"} 所 以 {"placeholder":"text_b"} 是 一 个 {"mask"} {"mask"} 实体.'
        label_words = [["转喻"], ["普通"]]
        myverbalizer = ManualVerbalizer(tokenizer, num_classes=2, label_words=label_words)
    logger.info('==============================')
    ...
    for epoch in range(args.total_epochs):
        ...
        if (epoch+1) % args.test_pre_epoch == 0:
            ...
            for step, inputs in enumerate(prog_bar_test):
                inputs.cuda()
                logits = prompt_model(inputs)
                labels = inputs['label']
            ...
            logger.info('==============================')
            meto_f1, literal_f1, acc, metonymic_prec, metonymic_rec, literal_prec, literal_rec = calculate_metof1_literal_acc_precisionandrecall(alllabels, allpreds)
            performances_results[epoch+1] = [acc, meto_f1, literal_f1, metonymic_prec, metonymic_rec, literal_prec, literal_rec]
            if best_acc < acc:
                best_acc = acc
                best_acc_epoch = epoch+1
            logger.info(f'[Epoch {epoch+1}] ===> Accuracy: {acc}, Metonymic_f1: {meto_f1}, Literal_f1: {literal_f1},'
                        f' metonymic_prec: {metonymic_prec}, metonymic_rec: {metonymic_rec}, literal_prec: {literal_prec}, literal_rec: {literal_rec}')
    # 测试.
    stats = "\n".join(["[Epoch {}] --> acc: {}, metoymic_f1: {}, literal_f1: {}, metonymic_prec: {}, metonymic_rec: {}, literal_prec: {}, literal_rec: {}".format(epoch, *performances_results[epoch]) for epoch in performances_results])
    ...
    logger.info(f'===> Best Accuracy: {best_acc}, Metonymic_f1: {performances_results[best_acc_epoch][1]}, '
                f'Literal_f1: {performances_results[best_acc_epoch][2]} (other metrics: {performances_results[best_acc_epoch][3:]}), At Epoch: {best_acc_epoch}')


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset", type=str, default='SemEval2007', choices=['ReLocaR', 'CoNLL2003', 'ChineseMR', 'SemEval2007'], help='specify which dataset, please refer to directory dataset_MR')

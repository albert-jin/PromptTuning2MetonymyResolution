# Continuous Template Construction and Continuous Answer Search based PromptMR
...
def experiment_prompt_base(args):
    if not args.log_file:
        logger = set_logger(f'../results_MR/ctc_promptMR_cas/{args.dataset}_{args.shot_count}_shot_@{datetime.now().strftime("%Y_%m_%d-%H_%M_%S")}.log')
    else:
    ...
    mytemplate = MixedTemplate(model=plm, tokenizer=tokenizer, text='{"placeholder":"text_a"} {"soft"} {"placeholder":"text_b"} {"soft"} {"mask"}.')
    ...
    for epoch in range(args.total_epochs):
        ...
        for step, inputs in enumerate(prog_bar):
            ...
            if step%200 ==0 and step>0 and args.shot_count>100:
                allpreds = []
                alllabels = []
                ...
                logger.info(
                    f'[{step}/{len(train_dataloader)} Epoch {epoch + 1}] ===> Accuracy: {acc}, Metonymic_f1: {meto_f1}, Literal_f1: {literal_f1}')
        # 保存进度条

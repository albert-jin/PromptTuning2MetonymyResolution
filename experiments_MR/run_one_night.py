from promptMR_cas import experiment_prompt_base
...
Experiment_Id = 0
benchmarks = ['ReLocaR', 'CoNLL2003', 'SemEval2007']  # 'ReLocaR',
parser = Parser()
for benchmark in benchmarks:
    ...
    if 'CoNLL2003' == benchmark:
        parser.total_epochs = 3
...
from ctc_promptMR_cas import experiment_prompt_base as experiment_prompt_base_ctc_cas
...
for benchmark in benchmarks:
    ...
    print(f'Experiment Id: {Experiment_Id} ===>', parser)
    Experiment_Id += 1
    experiment_prompt_base_ctc_cas(parser)
    experiment_prompt_base_ctc_cas(parser)
    experiment_prompt_base_ctc_cas(parser)
    # 小样本设置下模型效果
    for shot_num in [5, 10, 20, 50]:
